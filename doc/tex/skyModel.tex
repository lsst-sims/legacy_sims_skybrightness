...
XXX--add plots showing the dark-time residuals as a function of time-of-year and time-of-night for each filter.  See if there are any obvious residuals.--this might be getting wiped out by the way I zero-point the Canon maps? Ah, I save the zeropoint and mjd, so I can plot zeropoint as a function of time-of-night and season. Looks like the moon coming up is doing crazy things to the zeropoint. Probably need to put in a mask around the moon so it doesn't skew things.
