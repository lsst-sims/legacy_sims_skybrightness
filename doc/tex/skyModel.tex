...
We use an airmass grid of 1, 1.2, 1.4, 2.0, and 2.5 and a Healpixel grid with 192 elements (nside=4, resolution of $\sim15\degree$), resulting in 960 zodiacal template spectra.  The Healpixels are a grid in heliocentric ecliptic latitude and longitude (i.e., longitude zero is fixed to the solar position).  Some example zodiacal light spectra are shown in Figure~\ref{fig:zodiacal}. 
...
Technically, the airglow is also varies with the time of year and time of night.  This variation is quite small relative to the solar activity \citep{Noll12}, so we have not included it. Examples of the airglow spectra are shown in Figure~\ref{fig:airglow}.
