...
For each ESO model component, we save the output spectra as numpy zip files.  The model spectra run from 300 nm to 2 microns, with 0.1 nm stepsize.  In addition to the spectra, we also pre-compute the 6 LSST magnitudes from each spectrum at each grid point. These pre-computed spectra and magnitudes are stored in their own git repository (\url{https://github.com/lsst/sims\_skybrightness\_data}) and are about 1 Gb in size.
...
Technically, the airglow is also varies with the time of year and time of night.  This variation is quite small relative to the solar activity \citep{Noll12}, so we have not included it.  \citet{Patat08} also shows the seasonal variation in $BVRI$ has an amplitude of less than $\pm0.5$ magnitudes. Examples of the airglow spectra are shown in Figure~\ref{fig:airglow}.
...
One important lesson from the data is the counter-intuitive (at least to me) result that during twilight the darkest part of the sky is near zenith, not moderate airmass in the hemisphere facing away from the sun. One is better off observing at low airmass towards the sun rather than high airmass away from the sun.
...
%%  LocalWords:  radiative FORS healpix healpy pyEphem BVRI
